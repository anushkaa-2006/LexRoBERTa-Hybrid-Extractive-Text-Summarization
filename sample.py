...
def extractive_summary(text):
    """Selects key sentences from text using RoBERTa importance scoring, ensuring the summary is 1/4th the length of the original text."""
    sentences = sent_tokenize(text)
    num_sentences = max(1, len(sentences) // 4)  # Ensure at least one sentence is included
...
# Generate summaries and compute METEOR scores
meteor_scores = []
