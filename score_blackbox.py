...
blackbox_summaries_path = "blackbox_summary.csv"

# Read files
df_cnn = pd.read_csv(cnn_summaries_path)
df_chatgpt = pd.read_csv(blackbox_summaries_path)
...
# Extract first 80 summaries
cnn_summaries = df_cnn[expected_summary_column].tolist()[:80]
chatgpt_summaries = df_chatgpt[expected_summary_column].tolist()[:80]

# Ensure both lists have 80 entries
if len(cnn_summaries) < 80 or len(chatgpt_summaries) < 80:
    raise ValueError("Both summary files must contain at least 80 summaries.")
...
# Print results
print("\n\nBERTScore Results (CNN DailyMail Articles vs. BlackBox Summaries):")
